...
import { fetchProductDetails, fetchProductReviews, submitProductReview, deleteProductReview, fetchProducts, clearSelectedProduct } from '../../../store/productsSlice';
import { toggleWishlist } from '../../../store/wishlistSlice';
import { Star, Heart, Plus, Minus, MessageCircle, Share2, ShieldCheck, RefreshCw, AlertCircle, ChevronDown, ChevronUp, Leaf, Award, Trash2, Check } from 'lucide-react';
import ProductCard from '../../../components/ProductCard';
import api from '../../../utils/axiosConfig';
import { useNotification } from '../../../context/NotificationContext';
import { getImageUrl } from '../../../utils/imageConfig';

function ShopDetailsContent({ initialProduct }) {
  ...
  const productIdParam = slug; // Keeping variable name same for minimal change
  ...
  useEffect(() => {
    const handleRatingClickOutside = (e) => {
  ...
  if (!realProduct) {
    ...
    const handleReviewSubmit = (e) => {
      ...
      dispatch(submitProductReview({ productId: realProduct._id, rating, comment: comment.trim() }))
        ...
        .then(() => {
          ...
          setReviewError('');
          dispatch(fetchProductReviews(realProduct._id));
          dispatch(fetchProductDetails(realProduct._id));
        })
        .catch((err) => {
    ...
    const handleReviewDelete = async (reviewId) => {
      const isConfirmed = await showConfirm('Are you sure you want to permanently delete this customer review?');
      ...
      try {
        ...
        showAlert(err || 'Failed to delete review', 'error');
      }
    };
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '40px', marginBottom: '20px', alignItems: 'start' }}>
              ...
              <div className="glass" style={{ borderRadius: '24px', padding: '32px', background: 'white' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px', paddingBottom: '16px', borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <span style={{ fontSize: '13px', fontWeight: '700', color: '#374151' }}>{Number(realProduct.rating || 5).toFixed(1)}</span>
                ...
                <div style={{ background: '#F7FBFD', border: '1px solid rgba(221,244,255,0.8)', borderRadius: '16px', padding: '24px' }}>
                  ...
                  {user ? (
                    <form onSubmit={handleReviewSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      {/* Rating Selector */}
                      <div>
                        <label className="mg-form-label" style={{ marginBottom: '6px', display: 'block' }}>Rating</label>

                        {/* Custom Rating Dropdown & Interactive Stars */}
                        {/* Interactive Stars (Flipkart Style) */}
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                          ...
                          <span style={{ fontSize: '14px', fontWeight: '600', color: (hoverRating || rating) === 0 ? '#94a3b8' : ((hoverRating || rating) >= 3 ? '#3BAE56' : '#EF4444') }}>
  ...
  export default function ShopDetailsPage({ initialProduct }) {
    return (
      <Suspense fallback={
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
          <div style={{ width: '40px', height: '40px', border: '4px solid #f3f3f3', borderTop: '4px solid #3BAE56', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
        ...
        <ShopDetailsContent initialProduct={initialProduct} />
      </Suspense>
    )
  }
