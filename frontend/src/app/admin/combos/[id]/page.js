...
export default function AdminComboEditPage() {
  ...
  const fetchCombo = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        const comboImages = combo.images && Array.isArray(combo.images) && combo.images.length > 0 
          ? [...combo.images] 
          : (combo.image ? [combo.image] : []);

        setFormData({
          ...combo,
          image: combo.image || comboImages[0] || '',
          images: comboImages,
          components: (combo.components || []).map(c => ({
            ...
            quantity: c.quantity,
            label: c.label || '',
            _productData: c.product
          }))
        });
      }
    } catch (err) {
      showAlert('Failed to load combo', 'danger');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.put(`/combos/admin/${id}`, payload);
      if (res.data.success) {
        showAlert('Combo updated successfully', 'success');
        router.push('/admin/combos');
      }
    } catch (error) {
      showAlert(error.response?.data?.message || 'Failed to update combo', 'danger');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="admin-page animate__animated animate__fadeIn pb-5">
      <div className="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom border-secondary border-opacity-25">
        <div className="d-flex align-items-center gap-3">
          ...
          <div>
            <h2 className="fw-bold m-0" style={{ color: '#162C18' }}>Edit Combo</h2>
            <p className="text-muted m-0">Manage bundled items and multiple combo images.</p>
          </div>
        </div>
            ...
            <div className="card-body p-0">
              {formData.components.length === 0 ? (
                <div className="p-5 text-center text-muted">
                  ...
                  <h6>No products in this combo</h6>
                  <p className="fs-7">Click the "Add Product" button to add products.</p>
                </div>
              ) : (
            ...
            <div className="card-body p-4">
              ...
              <div>
                <input 
                  ...
                  disabled={uploading}
                />
                <label htmlFor="comboMultiImagesEdit" className="btn btn-outline-primary w-100 d-flex align-items-center justify-content-center gap-2 py-2" style={{ cursor: 'pointer' }}>
